...
**v0.5 stage 3b shipped this session** — 11 new tests + 1 retired (382 unit + 3 smoke = 385 total). `runtime/registry.py` with `scan_agents(path)` (one-level walk, sorts by name, skips invalid agent.yaml with warning so one broken agent doesn't blackhole startup). `GET /agents` endpoint returns name/version/description metadata only. `movate serve` CLI replaces the stub: takes `--host`, `--port`, `--agents-path` (env: `MOVATE_AGENTS_PATH`), `--log-level`; pre-inits storage on the parent loop, scans the agents path, runs uvicorn. **End-to-end binary smoke green**: `movate serve` starts → `curl /healthz` returns 200 → `movate auth create-key` mints a key → `curl /agents` lists the registered agents → `POST /run` returns 202 with job_id → `GET /jobs/{id}` returns the queued state. Worker (stage 4) is what makes those queued jobs actually execute.

1. [ ] **v0.5 stage 4: worker claim loop + `movate worker`** `[HIGH] [v0.5] [next] [≤2d]` — drain the queue via `claim_next_job`, dispatch agent vs workflow by `JobKind`, persist the result and update the job to terminal.
2. [ ] **v0.5 stage 5: PostgresProvider port** `[HIGH] [v0.5] [≤2d]` — same conformance suite, switches to `SELECT ... FOR UPDATE SKIP LOCKED` for the claim path.
