...
- [x] **Workflow checkpointer (memory / sqlite / postgres)** `[HIGH] [v1.1] [done]` — `TenantNamespacedCheckpointer` wraps LangGraph's `BaseCheckpointSaver` and prefixes every `thread_id` with the tenant tag (`tenant_id::thread_id`), so tenant A's workflow threads are invisible to tenant B even if thread IDs collide. All three backends ship via two APIs: `make_checkpointer(kind, *, tenant_id)` (sync; memory only) and `async_checkpointer(kind, *, tenant_id)` (async context manager; works for all three — handles the SQLite/Postgres connection-pool lifecycle around `ainvoke`). SQLite persists at `~/.movate/checkpoints.db` (override via `MOVATE_CHECKPOINT_DB`); Postgres uses `MOVATE_CHECKPOINT_PG_DSN` (or `MOVATE_DB_URL` as fallback). `WorkflowSpec.checkpointer` field on workflow.yaml wires into `compile_to_langgraph` automatically. 15 tests in [tests/test_workflow_checkpointer.py](tests/test_workflow_checkpointer.py) cover namespacing, factory error paths, end-to-end memory + SQLite through `compile_to_langgraph`, Postgres DSN-missing path, and tenant isolation across two workflows with the same `workflow_run_id`.
