...
- [x] **Throwaway IR→LangGraph prototype** `[HIGH] [v0.3] [done]` — built `scripts/langgraph_prototype.py`, validated four constructs (linear / conditional / parallel / HITL) against LangGraph 1.1.10, captured findings in [docs/langgraph-seam.md](docs/langgraph-seam.md), then promoted the linear case to production code at [src/movate/core/workflow/compilers/langgraph.py](src/movate/core/workflow/compilers/langgraph.py) and **deleted the spike**. Mitigates the #1 risk in the plan — the additive IR fields the prototype surfaced are now the documented v1.1 work plan.
...
- [x] **GH Actions eval-gate template (block on regression)** `[HIGH] [v1.0] [done]` — `cli/eval.py` gained `--baseline-file <path>` and `--output-baseline <path>` flags so baselines can be git-tracked instead of stuck in ephemeral runner sqlite. Template workflow at [examples/ci/eval-gate.yml](examples/ci/eval-gate.yml) ships a `gate-pr` job (PR runs `--baseline-file`, exits 1 on regression past tolerance) and a `refresh-baseline` job (main-merge re-runs eval with `--output-baseline` and auto-commits). Lives under `examples/` rather than `.github/workflows/` so it doesn't auto-execute in this repo — copy into your consumer repo's `.github/workflows/` to activate. Docs at [docs/ci-eval-gate.md](docs/ci-eval-gate.md). 6 tests covering load, write, mutual exclusion, malformed-JSON path.
...
- [~] **`workflow/compilers/langgraph.py`** `[HIGH] [v1.1] [in-progress: linear AGENT case shipped]` — alternative compiler from `WorkflowGraph` IR; gated by `workflow.yaml: runtime: langgraph` (default `homegrown`). **Linear AGENT workflows ship in v1.0** with full equivalence to the homegrown runner (same WorkflowResult, persistence, error short-circuit, tenant isolation). Conditional / parallel / HITL extensions follow as the additive IR fields land per [docs/langgraph-seam.md](docs/langgraph-seam.md) — the `can_compile` capability check refuses each unsupported construct with an operator-facing reason, so each follow-up PR flips one rejection branch to a compile branch. Spike `scripts/langgraph_prototype.py` deleted.
- [~] **Workflow checkpointer (memory / sqlite / postgres)** `[HIGH] [v1.1] [in-progress: memory shipped, sqlite + postgres deferred]` — `TenantNamespacedCheckpointer` wraps LangGraph's `BaseCheckpointSaver` and prefixes every `thread_id` with the tenant tag (`tenant_id::thread_id`), so tenant A's workflow threads are invisible to tenant B even if thread IDs collide. `make_checkpointer(kind, *, tenant_id)` factory selects memory / sqlite / postgres. `WorkflowSpec.checkpointer` field on workflow.yaml wires into `compile_to_langgraph` automatically. **v1.0**: memory backend complete (no infra; lost on restart; fine for linear/HITL local dev). **Deferred**: sqlite + postgres backends (require async connection-pool lifecycle around `ainvoke` — the runner needs to open/close the saver per workflow run). Both raise a factory-time `CheckpointerError` pointing at this BACKLOG entry today. 14 tests in [tests/test_workflow_checkpointer.py](tests/test_workflow_checkpointer.py) cover namespacing, factory error paths, end-to-end memory through `compile_to_langgraph`, and tenant isolation across two workflows with the same `workflow_run_id`.
...
- [x] **Conditional edges** `[HIGH] [v1.1] [done]` — `edges: [{from: A, to: B, kind: conditional, when: "$.score > 0.7"}]`. Hand-rolled JSONPath-like DSL at [src/movate/core/workflow/condition_dsl.py](src/movate/core/workflow/condition_dsl.py) — no `eval`, no third-party dep, parsed at compile time. Supports `== != < <= > >=`, `&& || !`, `in [...]`, nested JSONPath. `validate_for_runtime` dispatcher in compiler picks `validate_linear` (homegrown) vs `validate_conditional` (langgraph). Compiler emits `add_conditional_edges` with router fn that walks branches in YAML order. Default branch (`when: null`) required and must be last per source. 47 tests in [tests/test_workflow_conditional.py](tests/test_workflow_conditional.py).
- [x] **Parallel fan-out + reducer annotations** `[HIGH] [v1.1] [done]` — `kind: parallel_fan_out` / `parallel_fan_in` edges; LangGraph runs siblings concurrently and waits on fan-in upstreams. State-schema gains `x-movate-reducer: <name>` extension with six named reducers (append/union/max/min/last/merge) at [src/movate/core/workflow/reducers.py](src/movate/core/workflow/reducers.py). Compiler materialises a TypedDict from state_schema via [compilers/_typed_state.py](src/movate/core/workflow/compilers/_typed_state.py) when parallel edges are present; node fns return delta-only on that path. New `validate_dag` accepts both conditional and parallel; enforces no mixed-kinds per source + minimum-2-branches for fan-outs. 28 tests in [tests/test_workflow_parallel.py](tests/test_workflow_parallel.py).
