...
**v0.5 stage 5 shipped this session — Postgres port + production-ready storage.** 21 new tests (412 unit + 3 smoke = 415 total when PG enabled; 391/3 when not). `storage/postgres.py` with full Protocol parity to sqlite: JSONB columns, TIMESTAMPTZ, `SELECT ... FOR UPDATE SKIP LOCKED` for the claim path (lets multiple workers truly run in parallel — proven by a new test that two workers grab two distinct rows concurrently, vs sqlite's serialization). `build_storage()` switches on `MOVATE_DB_URL`; `tests/conftest.py` parametrizes the storage suite over `(memory, sqlite, postgres)` with PG skipped when `MOVATE_PG_TEST_URL` is unset. **End-to-end binary smoke against real Postgres green**: serve+worker against `postgresql://...`; POST /run, claim, execute, terminal — JSONB output column round-tripped via `output->>'message'`. **Bug surfaced + fixed**: asyncpg pool connections are bound to the event loop they're created on; the original `cli/serve.py` did `asyncio.run(storage.init())` then `uvicorn.run()`, putting the pool on a dead loop. Restructured to keep init + serve on one loop. Also flipped the auth middleware's fire-and-forget `touch_api_key` to inline await — race against asyncpg pool RESET path was triggering "another operation is in progress" 500s. **v0.5 capabilities now feature-complete on both backends.**

1. [ ] **Tag v0.5.0** `[HIGH] [v0.5] [next] [≤30m]` — full v0.5 stack landed; cut the release.
2. [ ] **GitHub Release artifact for v0.5.0** `[MED] [v0.5] [≤30m]` — `uv build` + `gh release create` per RELEASING.md Option A.
3. [ ] **CI: add a PG service-container job** `[MED] [post-v0.5] [≤2h]` — exercise the postgres conformance suite in CI, not just locally.
