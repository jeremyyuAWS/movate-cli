...
78. [x] **`POST /api/v1/agents/{name}/publish` — commit + push to GitHub** `[HIGH] [v0.7] [done 2026-05-13]` — Feature-flagged via `MDK_GITHUB_ENABLED=1` so the live runtime is unaffected until ops registers the GitHub App + sets the secrets. New `src/movate/integrations/github.py` module: `GitHubConfig.from_env`, JWT-RS256 App auth (hand-rolled signing — no PyJWT dep), 5-minute-buffer installation-token cache, 5-step Git Data API push (blobs → tree-with-base_tree → commit-with-parent → ref PATCH). One bundle = one commit; dot-prefixed files excluded. Endpoint advertises in `/openapi.json` regardless of flag state so the Angular client can generate against it now. 29 new tests via `httpx.MockTransport` — no live network. Idempotency (`no_changes: true`) deferred to a follow-up.

79. [x] **`GET /api/v1/agents/{name}/history`** `[HIGH] [v0.7] [done 2026-05-14]` — Feature-flagged via `MDK_GITHUB_ENABLED=1`, same gate as item 78. Added `list_history(target_dir, limit, page)` on `GitHubClient` — calls `GET /repos/.../commits?path=<dir>&per_page&page` with `per_page` clamped to GitHub's 100 max. New `CommitInfo` dataclass + `AgentCommitView` / `AgentHistoryView` Pydantic wire models. Endpoint flattens the nested GitHub response into the simple shape the UI wants (sha, message, author_name/email, timestamp, html_url). Empty commit lists return 200 with empty array (not 404 — "no history yet" is a valid state for a wizard-created agent). `has_more` heuristic surfaces a "Load more" button hint when the page is full. The `_api` helper got split into `_request` (Any return), `_api` (dict), `_api_list` (list) to support the new endpoint cleanly. 19 new tests (12 integration + 7 endpoint). `?since=<sha>` cursor deferred to a v0.8 follow-up.
...
115. [x] **Pre-deploy lockfile validator** `[MED] [v0.8] [~30min]` — Tier R. CI check that `uv lock --check` passes — fails if `pyproject.toml` declares deps not in `uv.lock`. Would have caught today's python-multipart drift before the failed deploy (cost ~30 min). One-line GH Actions step.
