...
6. [ ] **Server-side SMS/email notifications** `[MED] [post-v1.0] [2-3d]` — add `notify_channel` + `notify_target` columns to `jobs`; `NotificationDispatcher` Protocol + ACS / Twilio / SendGrid backends; worker fires-and-forgets on terminal. Local `--notify` desktop fallback (already shipped) covers the dev-team 90%; this is the customer-product path. Decision required: ACS vs Twilio vs SendGrid (cost / SSO / compliance trade-offs).
