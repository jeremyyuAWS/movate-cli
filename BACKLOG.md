...
**v0.5 stage 3a shipped this session** — 14 new HTTP tests (372 unit + 3 smoke = 375 total). `runtime/` package with `build_app(storage)` factory: `/healthz` (unauthed), `POST /run` (queues a job → 202 with `job_id`), `GET /jobs/{id}` (polls; tenant-scoped — cross-tenant lookups 404 not 403 to defeat job_id probing). Auth middleware (`runtime/middleware.py`) composes the stage-2 primitives; every failure mode collapses to a single `401 AUTH_REQUIRED` envelope (timing-oracle defense). Wire schemas (`runtime/schemas.py`) live separately from `core/models.py` so the API surface evolves independently of the DB schema. Tests cover all five auth failure modes uniformly returning 401, /run persisting tenant + key attribution onto the JobRecord, /jobs/{id} cross-tenant safety, and request validation (422 on missing fields / unknown kind / empty target). Stage 3b (`/agents` + `movate serve` CLI) lands next; stage 4 (worker) consumes the queue this populates.

1. [ ] **v0.5 stage 3b: `/agents` + `movate serve` CLI** `[HIGH] [v0.5] [next] [≤1d]` — agent registry scanner + uvicorn binding so the HTTP runtime is actually launchable.
